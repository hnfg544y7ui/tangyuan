...
#pragma bss_seg(".led7_driver.data.bss")
#pragma data_seg(".led7_driver.data")
#pragma const_seg(".led7_driver.text.const")
#pragma code_seg(".led7_driver.text")
#endif
...
void led7_scan(void *param)
{
    ...
    enum gpio_port led_port;
    u32 led_pin;
    if (__this->user_data->pin_type == LED7_PIN7) {
        //pin cnt output H
        led_port = __this->user_data->pin_cfg.pin7.pin[cnt] / 16;
        led_pin = BIT(__this->user_data->pin_cfg.pin7.pin[cnt] % 16);
        gpio_set_drive_strength(led_port, led_pin, PORT_DRIVE_STRENGT_64p0mA);
        gpio_set_mode(led_port, led_pin, PORT_OUTPUT_HIGH);
        for (i = 0; i < 7; i++) {
            if (seg & BIT(i)) {
                //pin i output L
                led_port = __this->user_data->pin_cfg.pin7.pin[i] / 16;
                led_pin = BIT(__this->user_data->pin_cfg.pin7.pin[i] % 16);
                gpio_set_drive_strength(led_port, led_pin, PORT_DRIVE_STRENGT_2p4mA);
                gpio_set_mode(led_port, led_pin, PORT_OUTPUT_LOW);
            }
        }
        cnt = (cnt >= 6) ? 0 : cnt + 1;
    } else if (__this->user_data->pin_type == LED7_PIN13) {
        //pin_comh  cnt output L
        led_port = __this->user_data->pin_cfg.pin13.pin_com[cnt] / 16;
        led_pin = BIT(__this->user_data->pin_cfg.pin13.pin_com[cnt] % 16);
        gpio_set_drive_strength(led_port, led_pin, PORT_DRIVE_STRENGT_64p0mA);
        gpio_set_mode(led_port, led_pin, PORT_OUTPUT_LOW);
        for (i = 0; i < 7; i++) {
            if (seg & BIT(i)) {
                //pin_segl i output H
                led_port = __this->user_data->pin_cfg.pin13.pin_seg[i] / 16;
                led_pin = BIT(__this->user_data->pin_cfg.pin13.pin_seg[i] % 16);
                gpio_set_drive_strength(led_port, led_pin, PORT_DRIVE_STRENGT_2p4mA);
                gpio_set_mode(led_port, led_pin, PORT_OUTPUT_HIGH);
            }
        }
        ...
        led_port = __this->user_data->pin_cfg.pin12.pin_comh[cnt] / 16;
        led_pin = BIT(__this->user_data->pin_cfg.pin12.pin_comh[cnt] % 16);
        gpio_set_drive_strength(led_port, led_pin, PORT_DRIVE_STRENGT_64p0mA);
        gpio_set_mode(led_port, led_pin, PORT_OUTPUT_HIGH);
        ...
        for (i = 0; i < 7; i++) {
            if (seg & BIT(i)) {
                //pin_segl i output L
                led_port = __this->user_data->pin_cfg.pin12.pin_segl[i] / 16;
                led_pin = BIT(__this->user_data->pin_cfg.pin12.pin_segl[i] % 16);
                gpio_set_drive_strength(led_port, led_pin, PORT_DRIVE_STRENGT_2p4mA);
                gpio_set_mode(led_port, led_pin, PORT_OUTPUT_LOW);
            }
        }
...
static void led7_timer_callback_func(u32 tid, void *private_data)
{
    led7_scan(NULL);
}
/*----------------------------------------------------------------------------*/
/**@brief   led7扫描初始化
   ...
   @return  void
   @author  Change.tsai
   @note    void led7_scan_timer_init(void)
*/
/*----------------------------------------------------------------------------*/
void led7_scan_timer_init()
{
    puts("led7_timer_init\n");
    int timer_tid = 0;
    static struct gptimer_config led7_timer_config;
    led7_timer_config.timer.period_us = 2 * 1000; //2ms
    led7_timer_config.irq_cb = led7_timer_callback_func; //设置中断回调函数
    led7_timer_config.mode = GPTIMER_MODE_TIMER;

#if(TCFG_LED7_RUN_RAM)
    led7_timer_config.irq_priority = 7;
#else
    led7_timer_config.irq_priority = 1;
#endif
    timer_tid = gptimer_init(TIMERx, &led7_timer_config);
    printf("led7 timer id:%d\n", timer_tid);
#if(TCFG_LED7_RUN_RAM)
    irq_unmask_set(timer_tid + 4, 1, 0); //设置不可屏蔽中断
#endif
