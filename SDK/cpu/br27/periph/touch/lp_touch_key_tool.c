...
static void res_receive_handle(void *_data, u16 len, bool rx)
{
    ...
    if (rx) {
        ...
        case BT_KEY_CH_RES_MSG:
            log_debug("ch0:%08d,  ch1:%08d,  ch2:%08d,  ch3:%08d,  ch4:%08d,  ch5:%08d,  ch6:%08d,  ch7:%08d,  ch8:%08d,  ch9:%08d\n",
                      ((data[1] >> 16) & 0xffff), (data[1] & 0xff),
                      ((data[2] >> 16) & 0xffff), (data[2] & 0xff),
                      ((data[3] >> 16) & 0xffff), (data[3] & 0xff),
                      ((data[4] >> 16) & 0xffff), (data[4] & 0xff),
                      ((data[5] >> 16) & 0xffff), (data[5] & 0xff));
            break;
...
int lp_touch_key_online_debug_key_event_handle(u8 ch_index, struct key_event *event)
{
...
static int lp_touch_key_debug_reinit(u8 update_state)
{
    ...
    case LP_KEY_ONLINE_ST_CH_RES_DEBUG_STOP:
        M2P_CTMU_CH_DEBUG_L = 0;
        M2P_CTMU_CH_DEBUG_H = 0;
        break;
    case LP_KEY_ONLINE_ST_CH_KEY_DEBUG_START:
        M2P_CTMU_CH_DEBUG_L = 0;
        M2P_CTMU_CH_DEBUG_H = 0;
        M2P_MESSAGE_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG0L + ch * 6) = ((lp_key_online.debug_cfg.cfg0) & 0xFF);
        M2P_MESSAGE_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG0H + ch * 6) = (((lp_key_online.debug_cfg.cfg0) >> 8) & 0xFF);
        M2P_MESSAGE_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG1L + ch * 6) = ((lp_key_online.debug_cfg.cfg1) & 0xFF);
        M2P_MESSAGE_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG1H + ch * 6) = (((lp_key_online.debug_cfg.cfg1) >> 8) & 0xFF);
        M2P_MESSAGE_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG2L + ch * 6) = ((lp_key_online.debug_cfg.cfg2) & 0xFF);
        M2P_MESSAGE_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG2H + ch * 6) = (((lp_key_online.debug_cfg.cfg2) >> 8) & 0xFF);
        break;
...
static int lp_touch_key_online_debug_parse(u8 *packet, u8 size, u8 *ext_data, u16 ext_size)
{
    ...
    case TOUCH_RECORD_GET_CH_SIZE:
        ...
        _size = res_data;
        _offset = 0;
        app_online_db_ack(parse_seq, (u8 *)&res_data, 4); //回复对应的通道数据长度
        break;
...
u8 lp_touch_key_testbox_remote_test(u8 ch, u8 event)
{
    u8 ret = true;
    u8 key_report = 0;
    if (event == CTMU_P2M_CH_FALLING_EVENT) {
        key_report = 0xF1;
        log_info("Notify testbox CH%d Down", ch);
    } else if (event == CTMU_P2M_CH_RAISING_EVENT) {
        key_report = 0xF2;
        log_info("Notify testbox CH%d Up", ch);
    } else if (event == CTMU_P2M_CH_SHORT_KEY_EVENT) {
    } else if (event == CTMU_P2M_CH_LONG_KEY_EVENT) {
    } else if (event == CTMU_P2M_CH_HOLD_KEY_EVENT) {
    } else {
        ret = false;
    }
    if (key_report) {
...
void lp_touch_key_testbox_inear_trim(u8 flag)
{
    ...
    if (flag == 1) {
        ...
        if (__this->config->eartch_ch < 8) {
            M2P_CTMU_CH_DEBUG_L |= BIT(__this->config->eartch_ch);
        } else {
            M2P_CTMU_CH_DEBUG_H |= BIT(__this->config->eartch_ch - 8);
        }
