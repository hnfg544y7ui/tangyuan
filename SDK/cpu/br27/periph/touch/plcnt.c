...
#pragma bss_seg(".plcnt.data.bss")
#pragma data_seg(".plcnt.data")
#pragma const_seg(".plcnt.text.const")
#pragma code_seg(".plcnt.text")
#endif
...
void plcnt_io_out(const struct touch_key_port *key)
{
    ...
    PLCNT_IO_PORTx->PU0  &= ~BIT(PLCNT_IO_xx);
    PLCNT_IO_PORTx->PD1  |=  BIT(PLCNT_IO_xx);
    PLCNT_IO_PORTx->OUT |=  BIT(PLCNT_IO_xx);
    PLCNT_IO_PORTx->DIR &= ~BIT(PLCNT_IO_xx);
}

void plcnt_io_in(const struct touch_key_port *key)
{
    if (key->port > IO_MAX_NUM) {
        return;
    } else if ((key->port / 16) == 0) {
    ...
    PLCNT_IO_PORTx->DIE |=  BIT(PLCNT_IO_xx);
    PLCNT_IO_PORTx->PU0  &= ~BIT(PLCNT_IO_xx);
    PLCNT_IO_PORTx->PD0  |=  BIT(PLCNT_IO_xx);
    PLCNT_IO_PORTx->OUT |=  BIT(PLCNT_IO_xx);
    PLCNT_IO_PORTx->DIR |=  BIT(PLCNT_IO_xx);
}

void plcnt_iomc(const struct touch_key_port *key)
{
    //放电计数引脚选择intputchannel2
    gpio_ich_sel_input_signal(key->port, INPUT_CH_SIGNAL_CAP, INPUT_CH_TYPE_GP_ICH);
