...
static JL_QDEC_TypeDef *rdec_get_reg(rdec_ch ch)
{
    ASSERT((u32)ch < RDEC_CH_MAX, "func:%s(), line:%d\n", __func__, __LINE__);
    JL_QDEC_TypeDef *reg = NULL;
    reg = (JL_QDEC_TypeDef *)(RDEC_BASE_ADDR + ch * RDEC_OFFSET);
    return reg;
}
...
static void rdec_reg_log_info(int rdec_cfg_id)
{
    ...
    JL_QDEC_TypeDef *rdec_reg = rdec_info[rdec_cfg_id]->rdecx;
...
static void rdec_cfg_info_load(int rdec_cfg_id)
{
    ...
    SFR(con, QDEC_MODE, 2, rdec_info[rdec_cfg_id]->rdec_cfg.mode);
    ...
    if (clk != (u8) - 1) {
        ...
        SFR(con, QDEC_SPND, 4, 0xe); //0.68ms  1364hz
    }
    /* SFR(smp, QDEC_SMP, 8, 13); */
...
{
    JL_QDEC_TypeDef *RDECx = rdec_get_reg(rdec_cfg->ch);
