...
#pragma bss_seg(".virtual_rtc.data.bss")
#pragma data_seg(".virtual_rtc.data")
#pragma const_seg(".virtual_rtc.text.const")
#pragma code_seg(".virtual_rtc.text")
#endif
...
{
    u64 actual_time;
    get_vir_actual_time(&actual_time);
    ...
    read_sys_time(&temp_time);
    temp_time.sec += sec;
    while (temp_time.sec >= 60) {
        temp_time.min++;
        temp_time.sec -= 60;
    }
    temp_time.min += min;
    while (temp_time.min >= 60) {
        temp_time.hour++;
        temp_time.min -= 60;
    }
    temp_time.hour += hour;
    while (temp_time.hour >= 24) {
        temp_time.day++;
        temp_time.hour -= 24;
        if (temp_time.day > __month_to_day(temp_time.year, temp_time.month)) {
            temp_time.day = 1;
