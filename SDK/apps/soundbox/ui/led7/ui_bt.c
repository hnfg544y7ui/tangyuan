...
#pragma bss_seg(".ui_bt.data.bss")
#pragma data_seg(".ui_bt.data")
#pragma const_seg(".ui_bt.text.const")
#pragma code_seg(".ui_bt.text")
#endif
#include "includes.h"
#include "ui/ui_api.h"
...
static int ui_bt_user(void *hd, void *private, int menu, int arg)//子界面显示 //返回true不继续传递 ，返回false由common统一处理
{
    int ret = true;
    LCD_API *dis = (LCD_API *)hd;
    if (!hd) {
