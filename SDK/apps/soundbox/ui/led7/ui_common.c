...
static void led7_show_volume(void *hd, u16 vol)
{
    LCD_API *dis = (LCD_API *)hd;
    dis->lock(1);
    dis->clear();
    ...
    if (vol > 99) { //显示三位时往前靠
        dis->show_char('V');
        dis->show_number(vol / 100);
        dis->show_number((vol / 10) % 10);
    } else {
        dis->show_char(' ');
        dis->show_char('V');
...
static void led7_show_eq(void *hd, s32 eq_index)
{
    LCD_API *dis = (LCD_API *)hd;
    dis->lock(1);
    dis->clear();
    dis->setXY(0, 0);
    dis->show_string((u8 *)"Eq ");
    dis->show_number(eq_index);
    dis->lock(0);
}

static void led7_show_mute(void *hd)
{
    LCD_API *dis = (LCD_API *)hd;
    dis->lock(1);
    dis->clear();
    dis->setXY(0, 0);
    dis->show_string((u8 *)"NVOL");
    dis->lock(0);
}
...
void ui_common(void *hd, void *private, u8 menu, u32 arg)//公共显示
{
    ...
    case MENU_POWER_UP:
        ...
        break;
    case MENU_MAIN_VOL:
        led7_show_volume(hd, arg & 0xffff);
        break;
    case MENU_WAIT:
        led7_show_wait(hd);
        break;
