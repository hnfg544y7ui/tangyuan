...
#pragma bss_seg(".ui_rtc.data.bss")
#pragma data_seg(".ui_rtc.data")
#pragma const_seg(".ui_rtc.text.const")
#pragma code_seg(".ui_rtc.text")
#endif
#include "ui/ui_api.h"
...
static int ui_rtc_user(void *hd, void *private, int menu, int arg)//子界面显示 //返回true不继续传递 ，返回false由common统一处理
{
    int ret = true;
    LCD_API *dis = (LCD_API *)hd;
    if (!dis) {
