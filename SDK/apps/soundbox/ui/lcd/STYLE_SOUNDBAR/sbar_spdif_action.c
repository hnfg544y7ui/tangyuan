...
#pragma bss_seg(".sbar_spdif_action.data.bss")
#pragma data_seg(".sbar_spdif_action.data")
#pragma const_seg(".sbar_spdif_action.text.const")
#pragma code_seg(".sbar_spdif_action.text")
#endif
#include "ui/ui.h"
...
{
    ui_hide(SPDIF_VOL_LAYOUT);
    ui_hide(SPDIF_EQ_LAYOUT);
    ui_hide(SPDIF_LAYOUT);
    ui_hide(SPDIF_MUTE_LAYOUT);
    ui_show(id);
}
/************************************************************
                        spdif模式主页窗口控件
************************************************************/
static int spdif_layout_check(int p)
{
    if (get_sys_aduio_mute_statu()) {
        if (ui_get_disp_status_by_id(SPDIF_MUTE_LAYOUT) <= 0) {
            ui_hide_other_and_show_one_layout(SPDIF_MUTE_LAYOUT);
        }
    } else {
        if (ui_get_disp_status_by_id(SPDIF_LAYOUT) <= 0) {
            ui_hide_other_and_show_one_layout(SPDIF_LAYOUT);
        }
    }
    return 0;
}
...
static int spdif_mode_onchange(void *ctr, enum element_change_event e, void *arg)
{
    ...
    case ON_CHANGE_INIT:
        puts("\n***spdif_mode_onchange***\n");
        ui_register_msg_handler(ID_WINDOW_SPDIF, ui_msg_handler);
        spdif_layout_check(0);//判断返回哪个布局
        break;
...
static void ui_show_vol_init(void)
{
    ...
    if (ui_get_cur_vol > 0 && ui_get_cur_vol < app_audio_get_max_volume()) {
        ui_hide(SPDIF_VOL_M_PIC); //隐藏最大最小图标
        ...
        ui_hide(SPDIF_VOL_PIC);
        ui_hide(SPDIF_VOL_NUM);

        //初始化不用再单独showXX_VOL_M_PIC
        struct ui_pic *pic = (struct ui_pic *)ui_core_get_element_by_id(SPDIF_VOL_M_PIC);
        if (ui_get_cur_vol) {
...
static void ui_show_vol_deal(u32 cur_vol)
{
    ...
    if (cur_vol > 0 && cur_vol < app_audio_get_max_volume()) {
        ...
        ui_hide(SPDIF_VOL_M_PIC);//隐藏最大最小控件

        //需要重新显示音量图标及大小
        ui_pic_show_image_by_id(SPDIF_VOL_PIC, 0);
        num = (struct ui_number *)ui_core_get_element_by_id(SPDIF_VOL_NUM);
        if (num) {
        ...
        if (cur_vol) {
            ui_pic_show_image_by_id(SPDIF_VOL_M_PIC, 0);
        } else {
            ui_pic_show_image_by_id(SPDIF_VOL_M_PIC, 1);
        }
    }
...
static int ui_music_vol_handler(const char *type, u32 arg)
{
    ...
    if (ui_get_disp_status_by_id(SPDIF_VOL_LAYOUT) <= 0) {
        ui_hide_other_and_show_one_layout(SPDIF_VOL_LAYOUT);
    }
    ...
    if (type && (!strcmp(type, "vol"))) {
        if (ui_get_disp_status_by_id(SPDIF_VOL_LAYOUT) == TRUE) {
...
static int ui_mute_sta_handler(const char *type, u32 arg)
{
    if (ui_get_disp_status_by_id(SPDIF_MUTE_LAYOUT) <= 0 && arg != 0) {
        ui_hide_other_and_show_one_layout(SPDIF_MUTE_LAYOUT);
    } else {
        if (ui_get_disp_status_by_id(SPDIF_LAYOUT) <= 0) {
            ui_hide_other_and_show_one_layout(SPDIF_LAYOUT);
...
static int spdif_mute_layout_onchange(void *ctr, enum element_change_event e, void *arg)
{
    struct layout *layout = (struct layout *)ctr;
    switch (e) {
    case ON_CHANGE_INIT:
...
REGISTER_UI_EVENT_HANDLER(SPDIF_MUTE_LAYOUT)
.onchange = spdif_mute_layout_onchange,
 .onkey = NULL,
  .ontouch = NULL,
};
...
/************************************************************
                         eq布局控件
 ************************************************************/
static u16 spdif_eq_timer = 0;
static void spdif_eq_timeout(void *p)
{
    int id = (int)(p);
    if (ui_get_disp_status_by_id(id) == TRUE) {
        spdif_layout_check(0);//判断返回哪个布局
...
static int ui_eq_sta_handler(const char *type, u32 arg)
{
    ...
    if (ui_get_disp_status_by_id(SPDIF_EQ_LAYOUT) <= 0) {
        ui_hide_other_and_show_one_layout(SPDIF_EQ_LAYOUT);
    }

    if (spdif_eq_timer) {
        sys_timer_modify(spdif_eq_timer, 3000); //继续显示,n毫秒后退出
    }

    if (type && (!strcmp(type, "eq"))) {
        if (ui_get_disp_status_by_id(SPDIF_EQ_LAYOUT) == TRUE) {
            num.type = TYPE_NUM;
            num.numbs = 1;
            num.number[0] = arg;
            ui_number_update_by_id(SPDIF_EQ_NUM, &num);
        }
    }
    return 0;
}

static int spdif_eq_layout_onchange(void *ctr, enum element_change_event e, void *arg)
{
    struct layout *layout = (struct layout *)ctr;
    switch (e) {
    case ON_CHANGE_INIT:
        if (!spdif_eq_timer) {
            spdif_eq_timer  = sys_timeout_add((void *)layout->elm.id, spdif_eq_timeout, 3000);
        }
        break;
    ...
    case ON_CHANGE_RELEASE:
        if (spdif_eq_timer) {
            sys_timeout_del(spdif_eq_timer);
            spdif_eq_timer = 0;
        }
        break;
    default:
...
REGISTER_UI_EVENT_HANDLER(SPDIF_EQ_LAYOUT)
.onchange = spdif_eq_layout_onchange,
 .onkey = NULL,
  .ontouch = NULL,
};
