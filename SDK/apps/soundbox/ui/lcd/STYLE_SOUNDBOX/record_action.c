...
#pragma bss_seg(".record_action.data.bss")
#pragma data_seg(".record_action.data")
#pragma const_seg(".record_action.text.const")
#pragma code_seg(".record_action.text")
#endif
#include "ui/ui.h"
...
static int record_layout_onkey(void *ctr, struct element_key_event *e)
{
    ...
    case UI_KEY_OK:
        ...
        break;
    case UI_KEY_ENC:
        /* app_task_put_key_msg(UI_KEY_ENC_START, 0); */
        break;
    case UI_KEY_UP:
        break;
    case UI_KEY_DOWN:
...
static void record_check_add(void *ptr)
{
...
#if 0
    extern u32 recorder_get_encoding_time();
    extern int recorder_is_encoding(void);
    if (true == recorder_is_encoding()) { //正在录音
        ...
        if (__this->start == 1 || (music_file_get_player_status(get_music_file_player()) && !__this->rec_play)) { //回放时显示录音盘符,名字,正在录音文本以及播放标志,从系统设置菜单返回也跑这里
            ui_highlight_element_by_id(REC_START);
            ui_text_show_index_by_id(REC_TEXT, 1);
            UI_MSG_POST("record_start:show_filename=%2:dev=%2", 1, logo);
            __this->start = 0;
