...
#pragma bss_seg(".fm_action.data.bss")
#pragma data_seg(".fm_action.data")
#pragma const_seg(".fm_action.text.const")
#pragma code_seg(".fm_action.text")
#endif
#include "ui/ui.h"
...
static int fm_layout_onkey(void *ctr, struct element_key_event *e)
{
    ...
    case UI_KEY_MENU:
        ...
        break;
    case UI_KEY_UP:
        if (get_total_mem_channel()) { //有电台记录时上一个电台
            app_send_message(APP_MSG_FM_PREV_STATION, 0);
        } else {
            app_send_message(APP_MSG_FM_PREV_FREQ, 0);
        }
        break;
    case UI_KEY_DOWN:
        if (get_total_mem_channel()) { //有电台记录时下一个电台
            app_send_message(APP_MSG_FM_NEXT_STATION, 0);
        } else {
            app_send_message(APP_MSG_FM_NEXT_FREQ, 0);
        }
        break;
...
static int fm_menu_list_onkey(void *ctr, struct element_key_event *e)
{
    ...
    case UI_KEY_OK:
        ...
        switch (sel_item) {
        case 0:
            app_send_message(APP_MSG_FM_SCAN_ALL, 0);
            ui_hide(FM_MENU_LAYOUT);
            break;
        case 1:
            if (get_total_mem_channel()) {
                fm_clear_all_station(); //清全部电台
            }
            ui_hide(FM_MENU_LAYOUT);
            break;
        case 2:
