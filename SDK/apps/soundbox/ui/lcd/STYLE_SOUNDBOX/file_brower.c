...
#pragma bss_seg(".file_brower.data.bss")
#pragma data_seg(".file_brower.data")
#pragma const_seg(".file_brower.text.const")
#pragma code_seg(".file_brower.text")
#endif
#include "ui/ui.h"
...
extern const int FATFS_LONG_NAME_ENABLE; //是否支持长文件名
...
static int file_select_enter(int from_index)
{
    ...
    if (__this->attr[i].attr == F_ATTR_DIR) { //判断是不是文件夹文件属性
        if (!from_index) {
            ...
            __this->fs = fscan_enterdir(__this->fs, (const char *)__this->text_list[i].fsname);
        }
        ...
        app_send_message(APP_MSG_MUSIC_PLAY_START_BY_SCLUST, __this->attr[i].sclust); //通过簇号播放
...
static int file_list_flush(int from_index)
{
    ...
    for (i = 0; i < sizeof(TEXT_FNAME_ID) / sizeof(TEXT_FNAME_ID[0]); i++) {
        memset((char *)__this->text_list[i].fname, 0, TEXT_NAME_LEN);
...
__find:
    i = from_index % one_page_num;
    if (from_index == 0) {
        sprintf((char *)__this->text_list[i].fname, "%s", "..");
        sprintf((char *)__this->text_list[i].fsname, "%s", "..");
        __this->attr[i].attr = F_ATTR_DIR;
        __this->text_list[i].unicode = 0;
        __this->text_list[i].len = strlen((char *)__this->text_list[i].fname);
    } else {
    ...
    if (f) {
        ...
        __this->text_list[i].len = fget_name(f, (u8 *)name_buf, TEXT_NAME_LEN);
    ...
    for (i = 0; i < sizeof(TEXT_FNAME_ID) / sizeof(TEXT_FNAME_ID[0]); i++) {
        if (__this->text_list[i].len) {
            if (__this->text_list[i].unicode) {
                ui_text_set_textw_by_id(TEXT_FNAME_ID[i], (char *)__this->text_list[i].fname, __this->text_list[i].len, FONT_ENDIAN_SMALL, FONT_DEFAULT | FONT_HIGHLIGHT_SCROLL);
            } else {
                ui_text_set_text_by_id(TEXT_FNAME_ID[i], (char *)__this->text_list[i].fname, __this->text_list[i].len, FONT_DEFAULT);
            }
            ...
            ui_text_set_text_by_id(TEXT_FNAME_ID[i], (const char *)__this->text_list[i].fname, __this->text_list[i].len, FONT_DEFAULT);
            if (ui_get_disp_status_by_id(PIC_FNAME_ID[i]) == true) {
...
static int file_browse_enter_onchane(void *ctr, enum element_change_event e, void *arg)
{
    ...
    case ON_CHANGE_INIT:
        if (!__this) {
            ...
            if (__this && FATFS_LONG_NAME_ENABLE) { //长文件名buf
                __this->lfn_buf = zalloc(512);
            }
        }
        ...
        if (!__this->fs) {
            ...
            __this->fs = fscan(dev_manager_get_root_path(dev), (const char *)MUSIC_SCAN_PARAM, 9);

            //支持长文件名
            if (__this->fs && FATFS_LONG_NAME_ENABLE) {
