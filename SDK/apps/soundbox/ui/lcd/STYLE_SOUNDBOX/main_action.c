...
#pragma bss_seg(".main_action.data.bss")
#pragma data_seg(".main_action.data")
#pragma const_seg(".main_action.text.const")
#pragma code_seg(".main_action.text")
#endif
#include "ui/ui.h"
...
static int menu_control_onkey(void *ctr, struct element_key_event *e)
{
    ...
    case UI_KEY_OK:
        ...
        if (sel_item == 0) {
            if (task_switch_check(APP_NAME_BT)) { //检测目标app是否可以切换
                if (cur) {
                    if (!strcmp(cur->name, APP_NAME_BT)) {//当前在目标app内，则只要显示出目标页面即可
                        ui_hide_main(ID_WINDOW_MAIN);//退出主页页面
                        ui_show_main(ID_WINDOW_BT);//显示目标页面
                        break;
                    }
                    /* else */
                    /* ui_hide_main(ID_WINDOW_MAIN); */
                }
                app_task_switch(APP_NAME_BT, ACTION_APP_MAIN, NULL);//切换模式 页面由app释放
            }
            break;
        }
    ...
    case UI_KEY_DOWN:
        ...
        if (sel_item == 3) {
            ...
            ui_text_show_index_by_id(MENU_TEXT, sel_item + 1);
        }
    ...
    case UI_KEY_POWER:
        ...
        /* power_off_deal(NULL, e->value - UI_KEY_POWER_START); */
        break;
    default:
