...
#if 1
#define log_info(x, ...)    printf("[LE-CLIENT]" x " ", ## __VA_ARGS__)
...
#define SMBOX_MULTI_ROLE_CLIENT      1
#define SUPPORT_MAX_CLIENT           2
...
void *le_smartbox_multi_ble_hdl = NULL;
void *le_multi_client_hdl[SUPPORT_MAX_CLIENT];
...
s8 mul_get_dev_index(u16 handle)
{
    s8 i;
    u16 *group_handle;
    u8 count;
...
static bool check_device_is_match(u8 info_type, u8 *data, int size)
{
    ...
    for (i = 0; i < CLIENT_MATCH_CONN_MAX; i++) {
        cfg = (client_match_cfg_t *)client_config->match_dev_cfg[i];
        if (cfg == NULL) {
            continue;
        }
        /* log_info("cfg = %08x\n",cfg);	 */
...
static void conn_pair_vm_do(struct pair_info_t *info, u8 rw_flag)
{
    ...
    if (rw_flag == 0) {
        ...
        if (!info) {
            ...
            put_buf((u8 *)conn_pair_info_table, unit_len * SUPPORT_MAX_CLIENT);
            put_buf((u8 *)info, unit_len);
...
static void check_target_uuid_match(search_result_t *result_info)
{
    ...
    for (i = 0; i < client_config->search_uuid_cnt; i++) {
        t_uuid = (target_uuid_t *)&client_config->search_uuid_table[i];
...
static void client_create_connection(u8 *conn_addr, u8 addr_type)
{
    ...
    set_ble_work_state(cur_dev_cid, BLE_ST_CREATE_CONN);
    log_info_hexdump((u8 *)create_conn_par, sizeof(struct create_conn_param_t));
...
void ble_multi_client_disconnect(void)
{
    ...
    for (u8 i = 0; i < SUPPORT_MAX_CLIENT; i++) {
        ...
        if (client_con_handle[i]) {
            OS_ENTER_CRITICAL();
            client_disconnect((void *)(intptr_t)i);
            OS_EXIT_CRITICAL();
...
int le_multi_client_hdl_init(u8 cid);
//尝试开新设备scan
int ble_enable_new_dev_scan(void)
{
    log_info("%s\n", __FUNCTION__);
...
static void smbox_client_cbk_sm_packet_handler(void *_hdl, uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    case HCI_EVENT_PACKET:
        ...
        case SM_EVENT_PASSKEY_INPUT_NUMBER:
            ...
            smbox_client_deal_passkey_input(&tmp32, event->con_handle);
    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_LE_META:
            ...
            case HCI_SUBEVENT_LE_ENHANCED_CONNECTION_COMPLETE: {
                ...
                ble_op_multi_att_send_conn_handle(client_con_handle[cur_dev_cid], cur_dev_cid, SMBOX_MULTI_ROLE_CLIENT);
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE: {
            ...
            ble_op_multi_att_send_conn_handle(0, tmp_index, SMBOX_MULTI_ROLE_CLIENT);
...
static int client_init_config(void *priv, const client_conn_cfg_t *cfg)
{
    ...
    client_config = (client_conn_cfg_t *)cfg;//reset config
    return APP_BLE_NO_ERROR;
}
...
void client_profile_init(void)
{
    ...
    //setup GATT client
    gatt_client_init();
    gatt_client_register_packet_handler(smbox_client_cbk_packet_handler);
    memset(&conn_pair_info, 0, sizeof(struct pair_info_t));
    memset(conn_pair_info_table, 0, sizeof(conn_pair_info_table));
    conn_pair_info.head_tag = BLE_VM_HEAD_TAG;
    conn_pair_info.tail_tag = BLE_VM_TAIL_TAG;
...
void smbox_bt_multi_client_init(void)
{
    ...
    u8 client_max_num = SUPPORT_MAX_CLIENT;
    memset(&client_con_handle, 0, client_max_num << 1);
...
int le_multi_client_hdl_init(u8 cid)
{
    ...
    app_ble_sm_event_callback_register(le_multi_client_hdl[cid], smbox_client_cbk_sm_packet_handler);
    return 0;
}
