...
#pragma bss_seg(".online_db_deal.data.bss")
#pragma data_seg(".online_db_deal.data")
#pragma const_seg(".online_db_deal.text.const")
#pragma code_seg(".online_db_deal.text")
#endif
...
#if (TCFG_AUDIO_DATA_EXPORT_DEFINE && (TCFG_AUDIO_DATA_EXPORT_DEFINE == AUDIO_DATA_EXPORT_VIA_SPP))
...
static int db_data_send_msg_evt = 0;
...
//协议栈发送完成，唤醒
static void db_wakeup_send_data(void)
{
    if (!db_active) {
        return;
    }
...
void db_data_recive_msg_timeout(void *priv)
{
    int evt_msg = *((int *)priv);
    app_online_event_handle(evt_msg);
}

static void db_data_send_msg(int evt_msg)
{
    db_data_send_msg_evt = evt_msg;
    sys_timeout_add((void *)&db_data_send_msg_evt, db_data_recive_msg_timeout, 1);
}
...
int app_online_db_send(db_pkt_e type, u8 *packet, u8 size)
{
    ...
    if (!db_data_write_sub((u8 *)&db_ptr, head_size, packet, size)) {
...
int app_online_db_ack(u8 seq, u8 *packet, u8 size)
{
    ...
    if (seq == 0xff) {
        if (!db_data_write_sub((u8 *)&db_ptr, 0, packet, size)) {
...
#endif
    if (!db_data_write_sub((u8 *)&db_ptr, 3, packet, size)) {
        log_error("send fail2,%d,%d", seq, size);
        return -1;
    }
    return 0;
...
static const u8 test_buffer_data[8] = {1, 2, 3, 4, 5, 6, 7, 8};
static void db_print_test_function(void)
{
    ...
    app_online_put_buf((u8 *)test_buffer_data, 32);
