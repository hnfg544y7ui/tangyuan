...
#ifndef MSD_BLOCK_NUM
#define MSD_BLOCK_NUM       1
#endif
#if TCFG_NANDFLASH_DEV_ENABLE
#undef MSD_BLOCK_NUM
#define MSD_BLOCK_NUM       16
#endif
#define MSD_BUFFER_SIZE     (MSD_BLOCK_NUM * 512)
...
static u8 msd_buf[MSD_BUFFER_SIZE * 2]  SEC(.usb.data.bss.exchange) __attribute__((aligned(64)));
#else
static u8 msd_buf[MSD_BUFFER_SIZE] SEC(.usb.data.bss.exchange) __attribute__((aligned(64)));
#endif
static struct usb_msd_handle _msd_handle SEC(.usb.data.bss.exchange);
#endif
...
static u32 msd_itf_hander(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    ...
    case USB_TYPE_CLASS:
        ...
        case USB_MSD_MAX_LUN:
            //请勿在这里初始化ep，主机在GET_MAX_LUN较短时间后使用bulk端点通信
            //会出现丢数据情况
...
static void mode_sense(const struct usb_device_t *usb_device)
{
    ...
    if (dev_ioctl(dev_fd, IOCTL_CHECK_WRITE_PROTECT, (u32)&sense) == 0) {
        if (sense) {
            scsi_mode_sense[2] = 0x80;
        } else {
            scsi_mode_sense[2] = 0x00;
        }
    }
...
static void msd_write_10_async(const struct usb_device_t *usb_device, u8 cur_lun, u32 lba, u16 lba_num)
{
    ...
    while (lba_num) {
        wdt_clear();
        num = lba_num > MSD_BLOCK_NUM ? MSD_BLOCK_NUM : lba_num;
        if (msd_handle->csw.uCSWDataResidue == 0) {
...
static void write_10(const struct usb_device_t *usb_device)
{
    ...
    while (lba_num) {
        ...
        if (err == 0) {
            stall_error(usb_device, 1, MEDIUM_ERROR);
            log_error("read usb error = %d, dev = %s", err, msd_handle->info.dev_name[cur_lun]);
            break;
        }
        dev_fd = check_disk_status(cur_lun);
        if (dev_fd) {
...
{
    if (!msd_handle) {
        //从pc前台切到cdc后台，有几率这个消息在关-开usb之后，已经释放了
        //msd_handle，在get_cbw()中用到msd_handle，会异常
        return;
    }
    const usb_dev usb_id = usb_device2id(usb_device);
#ifdef USB_HW_20
    ...
    if (private_scsi_cmd(usb_device, &(msd_handle->cbw))) {
        ...
        default:
            ...
            if (msd_handle->cbw.dCBWDataTransferLength) {
                stall_error(usb_device, !(msd_handle->cbw.bmCBWFlags & 0x80), 2);
            } else {
                msd_handle->csw.bCSWStatus = 1;
                msd_handle->info.bError = 2;
            }
            break;
        }
    }
    ...
    if (msd_handle) {
        for (i = 0; i < __get_max_msd_dev(); i++) {
            if (msd_handle->info.dev_handle[i]) {
                dev_close(msd_handle->info.dev_handle[i]);
                msd_handle->info.dev_handle[i] = NULL;
...
#if USB_MSD_BULK_DEV_USE_ASYNC
        msd_handle->msd_buf = (u8 *)dma_malloc(MSD_BUFFER_SIZE * 2);
#else
...
u32 msd_release()
{
    if (msd_handle) {
        ...
        if (msd_handle->ep_out_dmabuffer) {
            usb_free_ep_dmabuffer(0, msd_handle->ep_out_dmabuffer);
            msd_handle->ep_out_dmabuffer = NULL;
        }
#if USB_MALLOC_ENABLE
        if (msd_handle->msd_buf) {
#if USB_MSD_BULK_DEV_USE_ASYNC
            dma_free(msd_handle->msd_buf);
#else
