...
#pragma bss_seg(".RDA5807.data.bss")
#pragma data_seg(".RDA5807.data")
#pragma const_seg(".RDA5807.text.const")
#pragma code_seg(".RDA5807.text")
#endif
/*--------------------------------------------------------------------------*/
/**@file     rda5807.c
   @brief    RDA5807收音底层驱动
...
#if (TCFG_FM_RDA5807_ENABLE && TCFG_APP_FM_EN && TCFG_FM_OUTSIDE_ENABLE)
...
void rda5807_write_new(u8 num)
{
    u8 write_cnt = 0;
__rewriten:
    int ret = hw_i2c_master_write_nbytes_to_device(fm_dev_info->iic_hdl, 0x20, (u8 *)rda5807_dat, num);
    if (ret < 0) {
        if (write_cnt < 3) {
            write_cnt++;
            goto __rewriten;
...
void rda5807_read_new(u8 num)
{
    u8 read_cnt = 0;
__rereadn:
    int ret = hw_i2c_master_read_nbytes_from_device(fm_dev_info->iic_hdl, 0x21, read_dat, num);
    if (ret < 0) {
        if (read_cnt < 3) {
            read_cnt++;
            goto __rereadn;
...
void rda5807_write(u8 num)
{
    u8 re_write_cnt = 0;
__rewrite:
    int ret = app_IIC_write(RDA5807_WR_ADDRESS, 0xff, (u8 *)rda5807_dat, num);
    if (ret == 0) {
        // 写失败
        if (re_write_cnt <= 24) {
            re_write_cnt++;
...
u8 rda5807_iic_uninit(void *priv)
{
    fm_dev_info = (struct _fm_dev_info *)priv;
...
__reread:
        ...
        if (read_dat[8] == 0x58 && read_dat[9] == 0x04) {	//FM_RDA5807SP
            ...
            fm_type = 0;
            rda5807_iic_uninit(fm_dev_info);
            return 0;
        }
...
REGISTER_FM(rda5807) = {
    .logo          = "rda5807",
    .fm_iic_init   = rda5807_iic_init,
    .fm_iic_uninit = rda5807_iic_uninit,
    .init          = rda5807_init,
