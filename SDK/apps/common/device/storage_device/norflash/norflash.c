...
#pragma bss_seg(".norflash.data.bss")
#pragma data_seg(".norflash.data")
#pragma const_seg(".norflash.text.const")
#pragma code_seg(".norflash.text")
#endif
#include "norflash.h"
...
#define spi_cs_init() \
    do { \
        gpio_set_mode(IO_PORT_SPILT(_norflash.spi_cs_io), PORT_OUTPUT_HIGH); \
    } while (0)

#define spi_cs_uninit() \
    do { \
        gpio_set_mode(IO_PORT_SPILT(_norflash.spi_cs_io), PORT_HIGHZ); \
    } while (0)
...
int _norflash_open(void *arg)
{
    ...
    if (!_norflash.open_cnt) {
        spi_cs_init();
        spi_open(_norflash.spi_num, get_hw_spi_config(_norflash.spi_num));
