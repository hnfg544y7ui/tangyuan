...
static void *(*my_cJSON_malloc)(size_t sz) = my_malloc;
static void (*my_cJSON_free)(void *ptr) = my_free;
...
void my_cJSON_InitHooks(my_cJSON_Hooks *hooks)
{
    if (!hooks) { /* Reset hooks */
        my_cJSON_malloc = my_malloc;
        my_cJSON_free = my_free;
        return;
    }

    my_cJSON_malloc = (hooks->malloc_fn) ? hooks->malloc_fn : my_malloc;
    my_cJSON_free	 = (hooks->free_fn) ? hooks->free_fn : my_free;
}
...
int my_cJSON_IsFalse(const my_cJSON *const item)
{
    if (item == NULL) {
        return false;
    }

    return (item->type & 0xFF) == my_cJSON_False;
}

int my_cJSON_IsTrue(const my_cJSON *const item)
{
    if (item == NULL) {
        return false;
    }

    return (item->type & 0xff) == my_cJSON_True;
}


int my_cJSON_IsBool(const my_cJSON *const item)
{
    if (item == NULL) {
        return false;
    }

    return (item->type & (my_cJSON_True | my_cJSON_False)) != 0;
}

int my_cJSON_IsNull(const my_cJSON *const item)
{
    if (item == NULL) {
        return false;
    }

    return (item->type & 0xFF) == my_cJSON_NULL;
}

int my_cJSON_IsNumber(const my_cJSON *const item)
{
    if (item == NULL) {
        return false;
    }

    return (item->type & 0xFF) == my_cJSON_Number;
}

int my_cJSON_IsString(const my_cJSON *const item)
{
    if (item == NULL) {
        return false;
    }

    return (item->type & 0xFF) == my_cJSON_String;
}

int my_cJSON_IsArray(const my_cJSON *const item)
{
    if (item == NULL) {
        return false;
    }

    return (item->type & 0xFF) == my_cJSON_Array;
}

int my_cJSON_IsObject(const my_cJSON *const item)
{
    if (item == NULL) {
        return false;
    }

    return (item->type & 0xFF) == my_cJSON_Object;
}
...
my_cJSON *my_cJSON_AddArrayToObject(my_cJSON *const object, const char *const name)
{
    my_cJSON *array = my_cJSON_CreateArray();
    if (!array) {
        return NULL;
    }
    my_cJSON_AddItemToObject(object, name, array);
    return array;
}
