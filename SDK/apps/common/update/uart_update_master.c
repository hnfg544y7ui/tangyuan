...
#pragma bss_seg(".uart_update_master.data.bss")
#pragma data_seg(".uart_update_master.data")
#pragma const_seg(".uart_update_master.text.const")
#pragma code_seg(".uart_update_master.text")
#endif
...
static void update_process_run(void)
{
    ...
    while (1) {
        if (OS_NO_ERR != os_sem_pend(&__this->rx_sem, 800)) {
            ...
            if (__this->master_cur_baudrate != update_baudrate) {
                uart_update_set_baud(update_baudrate);
                __this->master_cur_baudrate = update_baudrate;
            }
            continue;
        }
...
static void update_loader_download_task(void *p)
{
    ...
    __this->master_cur_baudrate = 9600;
