...
#pragma bss_seg(".virfat_flash.data.bss")
#pragma data_seg(".virfat_flash.data")
#pragma const_seg(".virfat_flash.text.const")
#pragma code_seg(".virfat_flash.text")
#endif
...
#define AT_VOLATILE_RAM_CODE      // AT(.virfat_flash_code)

//要与fat_comm.exe -pad-backup2 -force-align-fat -out new_res.bin -image-size 8 对应
//要与fat_comm.exe -pad-backup2 -force-align-fat -out new_res.bin -image-size 16 对应
#if TCFG_VIRFAT_INSERT_FLASH_ENABLE
#define CONFIG_FAT_FLASH_SIZE     CONFIG_FLASH_SIZE
#else
#define CONFIG_FAT_FLASH_SIZE     16 * 1024 * 1024//CONFIG_EXTERN_FLASH_SIZE
#endif


// 如果需要使用16M以上的flash需要修改download.bat文件与之对应
#if (CONFIG_FAT_FLASH_SIZE > 8 * 1024 * 1024 /* FLASH_SIZE_8M */  )
#define OUTFLASH_FAT16_ENABLE 		1
#else /* #if (CONFIG_EXTERN_FLASH_SIZE > FLASH_SIZE_8M) */
#define OUTFLASH_FAT16_ENABLE 		0
#endif /* #if (CONFIG_EXTERN_FLASH_SIZE > FLASH_SIZE_8M) */
...
static void *fd;
static u32 real_capacity = 0;
static u32 jlfat_capacity = 0;

#define VIRFAT_FLASH_SET_DAMAGED_PART       0
#if VIRFAT_FLASH_SET_DAMAGED_PART
static u32 occupy_clust = 0;
void sdfile_fat_sub_maxclust(u32 clust);
#endif

#define VIRFAT_FLASH_FAST_WRITE     1
void virfat_flash_flush();

#if VIRFAT_FLASH_FAST_WRITE
static u8 *buf_write = NULL;
static u32 virfat_addr;
#endif
...
#if VIRFAT_FLASH_SET_DAMAGED_PART
    if (occupy_clust) {
        disk_handler->max_clust -= occupy_clust;
        occupy_clust = 0;
    }
...
{
    u8 *bpb_alloc = zalloc(512);
    if (bpb_alloc == NULL || offset > 512 || (offset + len) > 512) {
        memset(buf, 0, 512);
        return;
    }
    memcpy(bpb_alloc, bpb_data, 512);
    int a = 0;
    if (real_capacity >= 32 * 1024 * 1024 / 512) {
        fat_copy(&bpb_alloc[BPB_TotSec16], &a, 2);
        fat_copy(&bpb_alloc[BPB_TotSec32], &real_capacity, 4);
    } else {
        fat_copy(&bpb_alloc[BPB_TotSec16], &real_capacity, 2);
    }
    if (real_capacity >= 16 * 1024 * 1024 / 512) {
        ...
        fat_copy(&bpb_alloc[BPB_FATSz16], &fat_sector, 2);
        fat_copy(&bpb_alloc[BPB_RsvdSecCnt], &rsvdsec, 2);
    }
    put_buf((const u8 *)bpb_alloc, 96);
    memcpy(buf, bpb_alloc + offset, len);
    free(bpb_alloc);
...
static u32 virfat_flash_write(void *buf, u32 addr_sec, u32 len)
{
    ...
    if (addr_sec == 0) {
...
#if VIRFAT_FLASH_FAST_WRITE
        if (virfat_addr == 0) {
            virfat_addr = align_addr;
            virfat_read(fd, buf_write, virfat_addr, 4096);
        }
        if (virfat_addr != align_addr) {
            virfat_ioctl(fd, IOCTL_ERASE_SECTOR, virfat_addr);
            wlen = virfat_write(fd, buf_write, virfat_addr, 4096);
            if (wlen != 4096) {
        ...
        memcpy(buf_write + offset, buf, len);
        wlen = len;
#else
...
#if VIRFAT_FLASH_FAST_WRITE
    int align_addr = real_addrsec / 4096 * 4096;
    if (align_addr == virfat_addr) {
        virfat_flash_flush();
    }
#endif
...
u32 virfat_flash_get_real_capacity() //获取实际flash容量
{
    if (jlfat_capacity == 0) {
        ...
        /* jlfat_capacity = 8 * 1024 * 1024; */
#else
        if (jlfat_capacity < 16 * 1024 * 1024) {
#if VIRFAT_FLASH_SET_DAMAGED_PART
            y_printf(">>>[test]:real_capacity = 0x%x\n", jlfat_capacity);
            occupy_clust = CONFIG_FAT_FLASH_SIZE - jlfat_capacity;
            if (occupy_clust % 4096) {
                occupy_clust = occupy_clust / 4096 + 1;
            } else {
                occupy_clust = occupy_clust / 4096;
            }
            sdfile_fat_sub_maxclust(occupy_clust);
#endif
            /* jlfat_capacity = 16 * 1024 * 1024; */
        }
#endif
    ...
    for (int k = 0; k < dir_len; k += BUF_LEN) {
        ...
        for (int i = 0; i < BUF_LEN; i += 32) {
            ...
            if ((name_buf[0] != 0) && (name_buf[1] != 0)) {
                if (file_buf) {
                    memcpy((void *)((u32)file_buf + cnt * 12), name_buf, 12);
                }
                cnt++;
...
{
    u32 a = 0;
    virfat_flash_mount(&virfat_flash, (u8 *)bpb_data);
    real_capacity = virfat_flash_get_real_capacity();
    ...
    if (virfat_flash.capacity != real_capacity) {
        ...
        put_buf((const u8 *)bpb_alloc, 96);
        virfat_flash_mount(&virfat_flash, (u8 *) bpb_alloc);
    }
...
int flash_virfat_read(struct device *device, void *buf, u32 len, u32 addr)
{
    ...
    u32 _lba = addr;
    while (_len) {
        virfat_flash_read((void *)_buf, _lba, 512);
        //printf_buf(_buf, 512);
        _len -= 1;
        _lba += 1;
        _buf += 512;
...
static int flash_virfat_write(struct device *device, void *buf, u32 len, u32 addr)
{
    ...
    u32 _lba = addr;
    while (_len) {
        virfat_flash_write((void *)_buf, _lba, 512);
        //printf_buf(_buf, 512);
        _len -= 1;
        _lba += 1;
        _buf += 512;
...
static int flash_virfat_ioctrl(struct device *device, u32 cmd, u32 arg)
{
    ...
    case IOCTL_GET_STATUS:
        *(u32 *)arg = 1;
        break;
    case IOCTL_GET_ID:
        virfat_ioctl(fd, IOCTL_GET_ID, arg);
        /* *((u32 *)arg) = 0x12345678; */
        break;
...
static int _private_sfc_read(void *fd, void *buf, u32 addr, u32 len)
{
    ...
    u32 cpu_addr = sdfile_flash_addr2cpu_addr(flash_private_info->flash_private_addr + addr);
...
static int _private_sfc_write(void *fd, void *buf, u32 addr, u32 len)
{
    ...
    /* wlen = sfc_norflash_write(NULL, buf, len, addr); */
    return wlen;
}
...
static int private_sfc_open(const char *name, struct device **device, void *arg)
{
    ...
    /* } */
    virfat_get_cpu_fatfs_addr(arg, &flash_private_info->flash_private_addr, &flash_private_info->private_len);
