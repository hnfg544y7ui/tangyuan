...
* @brief 格式化接口
*
* @param path 路径 1.“sd0” 不管之前卡里面是什么文件系统，都会格式化成fat32 2.“storage/sd0/C” 格式化C分区，如果之前卡里面是fat12\16\32或者exfat ,格式化为对应的文件系统类型
* @param fs_type 文件系统类型
* @param clust_size 簇大小,簇为0时默认为卡本身簇大小。(簇大小和对应的文件系统需要对应、exfat只支持填0 不改变簇大小)
*
* @return 0格式化成功，非0  失败
*/
/* ----------------------------------------------------------------------------*/
 ...
 * -m  过滤文件夹
 * -f  加速按序号（或者时间排序序号）查找文件，使用为 “-fn”, n为数字(如：5、16、100)，消耗n * 16 RAM空间 , fscan_release 时释放
 */
 ...
 * @param path_dst 目标路径
 * @param newFile 新文件句柄
 * @param clr_attr 是否清除文件属性,改为可读写
 * @param path_len 路径buf长度，针对长文件名
 *
 * @return 0成功， 非0错误。
 */
/* ----------------------------------------------------------------------------*/
