...
#pragma bss_seg(".cvp_tool.data.bss")
#pragma data_seg(".cvp_tool.data")
#pragma const_seg(".cvp_tool.text.const")
#pragma code_seg(".cvp_tool.text")
#endif
...
const u8 audio_sdk_name[16] = "AC897X";
...
static int aec_online_nor_cmd(AEC_CFG *aec_cfg, void *_packet)
{
    ...
    if (packet->cmd == AEC_ONLINE_CMD_GETVER) {
        ...
        printf("AEC_ONLINE_CMD_GET_SECTION_NUM | AEC_ONLINE_CMD_GET_SECTION_NUM_NEW");
        struct _cmd {
            int id;
            int groupId;
        };
        ...
        printf("AEC_ONLINE_CMD_GLOBAL_GAIN_SUPPORT_FLOAT");
        ...
        printf("AEC_ONLINE_CMD_PASSWORD");
        uint8_t password = 0;
        if (password_en) {
            password = 1;
        }
        ...
        printf("AEC_ONLINE_CMD_VERIFY_PASSWORD");
        //check password
        int len = 0;
        char pass[64];
        ...
        printf("AEC_ONLINE_CMD_FILE_SIZE");
#if TCFG_AUDIO_TRIPLE_MIC_ENABLE
        /*3麦通话*/
        u32 cfg_file_size = sizeof(AEC_TMS_CONFIG);
#elif TCFG_AUDIO_DUAL_MIC_ENABLE
        /*双麦通话*/
#if (TCFG_AUDIO_DMS_SEL == DMS_NORMAL)
        u32 cfg_file_size = sizeof(AEC_DMS_CONFIG) - 27;//-27是为了兼容新增的参数
#else/*TCFG_AUDIO_DMS_SEL == DMS_FLEXIBLE*/
        u32 cfg_file_size = sizeof(DMS_FLEXIBLE_CONFIG) - 3;//-3是为了兼容新增的参数
#endif/*TCFG_AUDIO_DMS_SEL*/
#else/*SINGLE MIC*/
        u32 cfg_file_size = sizeof(AEC_CONFIG) - 6;//-6是为了兼容新增的参数
#endif/*TCFG_AUDIO_DUAL_MIC_ENABLE*/
        y_printf("get aec file_size : %d", cfg_file_size);
        ci_send_packet_new(aec_cfg, id, (u8 *)&cfg_file_size, sizeof(u32));
        return 0;
    } else if (packet->cmd == AEC_ONLINE_CMD_FILE) {
        printf("AEC_ONLINE_CMD_FILE");
        put_buf((const u8 *)packet, 64);
        struct cmd {
            int id;
            int modeId;
            int offset;
        ...
        AEC_CONFIG cfg_file;
#endif/*TCFG_AUDIO_DUAL_MIC_ENABLE*/
        int version = cmd.modeId;
        printf("version %x", version);
        int cfg_file_len = get_aec_config((u8 *)&cfg_file, version);
        y_printf("send_data_len %d", cmd.count);
        /*数据大于128字节需要分包发送*/
        ci_send_packet_new(aec_cfg, id, ((u8 *)&cfg_file) + cmd.offset, cmd.count);
        return 0;//-EINVAL;
    } else if (packet->cmd == AEC_ONLINE_CMD_MODE_COUNT) {
        printf("AEC_ONLINE_CMD_MODE_COUNT");
        //模式个数
        ...
        printf("AEC_ONLINE_CMD_MODE_NAME");
        //utf8编码得名字
        struct cmd {
            int id;
            int modeId;
        ...
        u8 tmp[32] = {0};
        u8 name_len =  strlen((const char *)aec_tool_tab[cmd.modeId].mode_name);
        ...
        printf("AEC_ONLINE_CMD_MODE_GROUP_COUNT");
        struct cmd {
            int id;
            int modeId;
        };
        ...
        printf("AEC_ONLINE_CMD_MODE_GROUP_RANGE");
        struct cmd {
            int id;
            int modeId;
            int offset;
        ...
        printf("AEC_ONLINE_CMD_EQ_GROUP_COUNT");
        u32 eq_group_num = 0;
        ...
        printf("AEC_ONLINE_CMD_EQ_GROUP_RANGE");
        struct cmd {
            int id;
            int modeId;
        };
...
{
#if TCFG_AUDIO_TRIPLE_MIC_ENABLE
    cvp_mode_set(AEC_CFG_TMS_DNS);
#elif TCFG_AUDIO_DUAL_MIC_ENABLE
#if (TCFG_AUDIO_DMS_SEL == DMS_NORMAL)
#if (TCFG_AUDIO_CVP_NS_MODE == CVP_ANS_MODE)
    cvp_mode_set(AEC_CFG_DMS);
#else/*CVP_DNS_MODE*/
    cvp_mode_set(AEC_CFG_DMS_DNS);
#endif/*TCFG_AUDIO_CVP_NS_MODE*/
#else/*TCFG_AUDIO_DMS_SEL == DMS_FLEXIBLE*/
#if (TCFG_AUDIO_CVP_NS_MODE == CVP_ANS_MODE)
    cvp_mode_set(AEC_CFG_DMS_FLEXIBLE);
#else/*CVP_DNS_MODE*/
    cvp_mode_set(AEC_CFG_DMS_FLEXIBLE_DNS);
#endif/*TCFG_AUDIO_CVP_NS_MODE*/
#endif/*TCFG_AUDIO_DMS_SEL*/
#else/*SINGLE MIC*/
#if (TCFG_AUDIO_CVP_NS_MODE == CVP_ANS_MODE)
    cvp_mode_set(AEC_CFG_SMS);
#else/*CVP_DNS_MODE*/
    cvp_mode_set(AEC_CFG_SMS_DNS);
#endif/*TCFG_AUDIO_CVP_NS_MODE*/
#endif/*TCFG_AUDIO_DUAL_MIC_ENABLE*/
