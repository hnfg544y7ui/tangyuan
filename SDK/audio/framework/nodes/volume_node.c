...
static void volume_ioc_start(struct volume_hdl *hdl)
{
    ...
    if (!len) {
        log_debug("%s, read node data err\n", __FUNCTION__);
    }
...
static int volume_ioc_update_parm(struct volume_hdl *hdl, int parm)
{
    ...
    default:
        ...
        log_debug("parm update failed : %x\n", value);
        break;
    }
    return ret;
}
