...
#pragma bss_seg(".audio_capture.data.bss")
#pragma data_seg(".audio_capture.data")
#pragma const_seg(".audio_capture.text.const")
#pragma code_seg(".audio_capture.text")
#endif
/*
 ****************************************************************************
...
extern int aec_data_export_init(u8 ch);
    ...
    ((TCFG_AUDIO_DATA_EXPORT_DEFINE == AUDIO_DATA_EXPORT_VIA_SPP) \
...
#if (TCFG_AUDIO_DATA_EXPORT_DEFINE == AUDIO_DATA_EXPORT_VIA_SD)
    u32 sd_write_sector;
    OS_SEM sd_sem;
...
#endif/*AUDIO_DATA_EXPORT_VIA_SPP*/
...
#if (TCFG_AUDIO_TCFG_AUDIO_DATA_EXPORT_DEFINE == AUDIO_DATA_EXPORT_VIA_SPP)
#if (SPP_EXPORT_CH > 1)
...
int audio_mic_init(u16 sr)
{
    ...
    if (adc_dm) {
        u8 ch = TCFG_SPP_DATA_EXPORT_ADC_MIC_CHA;
        ...
        for (int i = 0; i < AUDIO_ADC_MAX_NUM; i++) {
            if (ch & AUDIO_ADC_MIC(i)) {
                adc_file_mic_open(&adc_dm->mic_ch, AUDIO_ADC_MIC(i));
            }
        }
...
static PLNK_PARM *pdm_mic = NULL;
extern int pdm_mic_file_param_init(PLNK_PARM *pdm_mic);
 ...
 */
static void plnk_mic_output(void *priv, void *buf, u32 len)
{
    s16 *mic0;
    s16 *mic1;
    PLNK_PARM *plnk = (PLNK_PARM *)priv;
    u8 ch_num = plnk->ch_num;
...
static int audio_pdm_mic_init(u16 sr)
{
    ...
    pdm_mic = zalloc(sizeof(PLNK_PARM));

    if (pdm_mic) {
        pdm_mic->sr = sr;
        pdm_mic->dma_len = 512;
        pdm_mic->isr_cb = plnk_mic_output;
        pdm_mic->private_data = pdm_mic;	//保存私有指针

        int len = pdm_mic_file_param_init(pdm_mic);
        pdm_mic = plnk_init(pdm_mic);
        plnk_start(pdm_mic);
    }
    return 0;
}
static int audio_pdm_mic_exit()
{
    if (pdm_mic) {
        plnk_uninit(pdm_mic);
        free(pdm_mic);
...
int audio_capture_start(void)
{
    ...
    memset(&de, 0, sizeof(data_export_t));
#if SPP_SEND_BY_TIMER
    de.send_timer = sys_timer_add(NULL, aec_export_timer, SPP_SEND_INTERVAL);
