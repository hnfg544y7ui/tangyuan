    ...
    if (mic_demo && mic_demo->adc_2_dac) {
        if (adc_hdl.channel_num >= 2) {
            ...
            for (int i = 0; i < len / 2; i++) {
                temp[2 * i] = data[adc_hdl.channel_num * i];
                temp[2 * i + 1] = data[adc_hdl.channel_num * i + 1];
            }
            ...
            for (int i = 0; i < (len / 2); i++) {
                switch (mic_ch_sel) {
                case 0:
                    mono_data[i] = data[i * adc_hdl.channel_num];			/*第一路 mic采样的数据*/
                    break;
                case 1:
                    mono_data[i] = data[i * adc_hdl.channel_num + 1];	     /*第一路 mic采样的数据*/
                    break;
                case 2:
                    mono_data[i] = data[i * adc_hdl.channel_num + 2];		 /*第三路 mic采样的数据*/
                    break;
                case 3:
                    mono_data[i] = data[i * adc_hdl.channel_num + 3];	     /*第四路 mic采样的数据*/
                    break;
                default:
                    break;
                }
            ...
            s16 dual_data[len * 2];
            for (int i = 0; i < len; i++) {
                dual_data[i] = data[i / 2];
            }
            wlen = audio_dac_write(&dac_hdl, dual_data, len * 2);
#else
            /*输出一个mic的数据到DAC一个通道*/
            putchar('d');
...
void audio_adc_mic_demo_open(u8 mic_idx, u8 gain, u16 sr, u8 mic_2_dac)
{
    ...
    if (mic_demo) {
        ...
        if (mic_idx & AUDIO_ADC_MIC_0) {
            mic_param[0].mic_mode      = AUDIO_MIC_CAP_MODE;
            mic_param[0].mic_ain_sel   = AUDIO_MIC0_CH0;
            mic_param[0].mic_bias_sel  = AUDIO_MIC_BIAS_CH0;
            mic_param[0].mic_bias_rsel = 4;
            mic_param[0].mic_dcc       = 8;
        ...
        if (mic_idx & AUDIO_ADC_MIC_1) {
            mic_param[1].mic_mode      = AUDIO_MIC_CAP_MODE;
            mic_param[1].mic_ain_sel   = AUDIO_MIC1_CH0;
            mic_param[1].mic_bias_sel  = AUDIO_MIC_BIAS_CH1;
            mic_param[1].mic_bias_rsel = 4;
            mic_param[1].mic_dcc       = 8;
            audio_adc_mic_open(&mic_demo->mic_ch, AUDIO_ADC_MIC_1, &adc_hdl, &mic_param[1]);
            audio_adc_mic_set_gain(&mic_demo->mic_ch, AUDIO_ADC_MIC_1, gain);
            audio_adc_mic_gain_boost(AUDIO_ADC_MIC_1, 0);
        }
        if (mic_idx & AUDIO_ADC_MIC_2) {
            mic_param[2].mic_mode      = AUDIO_MIC_CAP_MODE;
            mic_param[2].mic_ain_sel   = AUDIO_MIC2_CH0;
            mic_param[2].mic_bias_sel  = AUDIO_MIC_BIAS_CH2;
            mic_param[2].mic_bias_rsel = 4;
            mic_param[2].mic_dcc       = 8;
            audio_adc_mic_open(&mic_demo->mic_ch, AUDIO_ADC_MIC_2, &adc_hdl, &mic_param[2]);
            audio_adc_mic_set_gain(&mic_demo->mic_ch, AUDIO_ADC_MIC_2, gain);
            audio_adc_mic_gain_boost(AUDIO_ADC_MIC_2, 0);
        }
        if (mic_idx & AUDIO_ADC_MIC_3) {
            mic_param[3].mic_mode      = AUDIO_MIC_CAP_MODE;
            mic_param[3].mic_ain_sel   = AUDIO_MIC3_CH0;
            mic_param[3].mic_bias_sel  = AUDIO_MIC_BIAS_CH3;
            mic_param[3].mic_bias_rsel = 4;
            mic_param[3].mic_dcc       = 8;
            audio_adc_mic_open(&mic_demo->mic_ch, AUDIO_ADC_MIC_3, &adc_hdl, &mic_param[3]);
            audio_adc_mic_set_gain(&mic_demo->mic_ch, AUDIO_ADC_MIC_3, gain);
            audio_adc_mic_gain_boost(AUDIO_ADC_MIC_3, 0);
        }
#endif
        //step1:设置mic通道采样率
...
void audio_adc_linein_demo_open(u8 linein_idx, u8 gain, u16 sr, u8 linein_2_dac)
{
    ...
    if (linein_demo) {
        ...
        if (linein_idx & AUDIO_ADC_LINE2) {
            adc_file_linein_open(&linein_demo->linein_ch, AUDIO_ADC_LINE2);
            linein_demo->linein_ch_num += 1;
        }
        if (linein_idx & AUDIO_ADC_LINE3) {
            adc_file_linein_open(&linein_demo->linein_ch, AUDIO_ADC_LINE3);
            linein_demo->linein_ch_num += 1;
        }
#else
        ...
        if (linein_idx & AUDIO_ADC_LINE0) {
            ...
            audio_adc_linein_gain_boost(AUDIO_ADC_LINE0, 0);
            linein_demo->linein_ch_num += 1;
        }
        if (linein_idx & AUDIO_ADC_LINE1) {
            ...
            audio_adc_linein_gain_boost(AUDIO_ADC_LINE1, 0);
            linein_demo->linein_ch_num += 1;
        }
        if (linein_idx & AUDIO_ADC_LINE2) {
            linein_param.linein_ain_sel   = AUDIO_LINEIN2_CH0;
            linein_param.linein_dcc       = 14;
            audio_adc_linein_open(&linein_demo->linein_ch, AUDIO_ADC_LINE2, &adc_hdl, &linein_param);
            audio_adc_linein_set_gain(&linein_demo->linein_ch, AUDIO_ADC_LINE2, gain);
            audio_adc_linein_gain_boost(AUDIO_ADC_LINE2, 0);
            linein_demo->linein_ch_num += 1;
        }
        if (linein_idx & AUDIO_ADC_LINE3) {
            linein_param.linein_ain_sel   = AUDIO_LINEIN3_CH0;
            linein_param.linein_dcc       = 14;
            audio_adc_linein_open(&linein_demo->linein_ch, AUDIO_ADC_LINE3, &adc_hdl, &linein_param);
            audio_adc_linein_set_gain(&linein_demo->linein_ch, AUDIO_ADC_LINE3, gain);
            audio_adc_linein_gain_boost(AUDIO_ADC_LINE3, 0);
            linein_demo->linein_ch_num += 1;
        }
#endif
...
*********************************************************************
*                  AUDIO AD2DA DEMO
* Description: 通过AD2DA模块实现ADC 0/1通道分别对DAC FL/FR通道的直通。
* Arguments  : None.
* Return     : None.
* Note(s)    : 每次调用audio_ad2da_open接口需要定义ADC的通道(单选),ADC通道使用的模拟接口(单选),DAC输出通道(可通过或运算多选)
*********************************************************************
*/
void audio_ad2da_demo(void)
{
    audio_ad2da_open(AUDIO_ADC_LINE0, AUDIO_LINEIN0_CH0, DAC_CH_FL);
    audio_ad2da_open(AUDIO_ADC_LINE1, AUDIO_LINEIN1_CH0, DAC_CH_FR);
